...
# APPLY BACKGROUND FIRST - BEFORE ANY OTHER STYLING
st.markdown("""
<style>
html, body {
    background: linear-gradient(180deg, rgba(40,15,4,0.7) 0%, rgba(0,0,0,0.5) 50%, rgba(40,15,4,0.7) 100%), 
                url('https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=1600&h=900&fit=crop') center/cover fixed !important;
    margin: 0 !important;
    padding: 0 !important;
...
st.set_page_config(page_title="FitPlan Pro - Non-Veg Diet", page_icon="🍗",
...
body { color:#fff!important; font-family:'DM Sans',sans-serif!important; }
...
.stButton>button { background:linear-gradient(135deg,#ea580c,#c2410c)!important;
  ...
  box-shadow:0 4px 18px rgba(234,88,12,0.40)!important; transition:all 0.20s!important; }
.stButton>button:hover { transform:translateY(-2px)!important; box-shadow:0 6px 28px rgba(234,88,12,0.65)!important; }

.stTabs [data-baseweb="tab-list"] { background:rgba(40,15,4,0.80)!important; border-radius:10px!important;
  padding:4px!important; border:1px solid rgba(249,115,22,0.25)!important; }
...
.stTabs [aria-selected="true"] { background:linear-gradient(135deg,#ea580c,#c2410c)!important;
  color:#fff!important; box-shadow:0 3px 12px rgba(234,88,12,0.50)!important; }
.stCheckbox>label { color:#fff!important; }

div[data-testid="stHorizontalBlock"]:first-of-type div[data-testid="stButton"]>button {
  background:rgba(234,88,12,0.22)!important; border:2px solid #ea580c!important;
  ...
  box-shadow:0 2px 10px rgba(234,88,12,0.25)!important; transition:all 0.15s ease!important; }
div[data-testid="stHorizontalBlock"]:first-of-type div[data-testid="stButton"]>button:hover {
  background:rgba(234,88,12,0.50)!important; transform:translateY(-1px)!important; }
...
.nav-logo-txt { font-family:'Bebas Neue',sans-serif; font-size:1.5rem; letter-spacing:5px;
  color:#f97316; text-shadow:0 0 20px rgba(249,115,22,0.60); line-height:1; }
.meal-card { background:rgba(35,12,3,0.88); border:1.5px solid rgba(249,115,22,0.28);
...
.meal-label { font-size:0.58rem; font-weight:700; letter-spacing:2.5px; text-transform:uppercase;
  color:rgba(249,115,22,0.90); margin-bottom:8px; display:flex; align-items:center; gap:6px; }
.meal-text { font-size:0.90rem; color:rgba(255,255,255,0.88); line-height:1.65; }
.tip-card { background:rgba(35,12,3,0.75); border:1px solid rgba(249,115,22,0.22);
...
.progress-bar-fill { height:100%; background:linear-gradient(90deg,#f97316,#ea580c); border-radius:3px; }
...
with _n[1]:
    if st.button("Dashboard", key="nd_db", use_container_width=True): st.switch_page("pages/2_Dashboard.py")
with _n[2]:
    if st.button("Profile",   key="nd_pr", use_container_width=True): st.switch_page("pages/1_Profile.py")
with _n[3]:
    if st.button("Diet Menu", key="nd_dm", use_container_width=True): st.switch_page("pages/4_Diet_Plan.py")
with _n[4]:
    if st.button("Workout",   key="nd_wp", use_container_width=True): st.switch_page("pages/3_Workout_Plan.py")
with _n[5]:
    if st.button("AI Coach",  key="nd_ai", use_container_width=True):
        try: st.switch_page("pages/5_ai_coach.py")
        except Exception as e: st.warning(str(e))
with _n[6]:
    if st.button("Records",   key="nd_rc", use_container_width=True):
        try: st.switch_page("pages/6_records.py")
        except Exception as e: st.warning(str(e))
with _n[7]:
    if st.button("Sign Out",  key="nd_so", use_container_width=True):
        logout(uname)
...
st.markdown(
    "<div style='background:linear-gradient(135deg,rgba(50,18,4,0.92),rgba(65,25,5,0.78) 60%,rgba(35,10,3,0.94));"
    "border:1.5px solid rgba(249,115,22,0.40);border-radius:20px;padding:36px 44px;margin-bottom:28px;"
    ...
    "letter-spacing:2px;color:rgba(249,115,22,0.80);background:rgba(249,115,22,0.12);"
    "border:1px solid rgba(249,115,22,0.30);border-radius:20px;padding:4px 14px'>&#127829; Non-Vegetarian</div>"
    "<div style='font-size:0.62rem;font-weight:700;letter-spacing:4px;text-transform:uppercase;"
    "color:rgba(249,115,22,0.80);margin-bottom:10px'>Personalised Nutrition Plan</div>"
    ...
    + uname_up + "'s <span style='color:#f97316'>Diet Plan</span></div>"
    "<div style='display:flex;gap:16px;flex-wrap:wrap;margin-bottom:16px'>"
    "<span style='background:rgba(249,115,22,0.15);border:1px solid rgba(249,115,22,0.35);"
    "border-radius:20px;padding:4px 14px;font-size:0.78rem;font-weight:600;color:#f97316'>"
    "&#127829; Non-Vegetarian</span>"
    ...
    with tab:
        ...
        with col_hdr:
            ...
            st.markdown("<div style='font-size:0.62rem;font-weight:700;letter-spacing:3px;text-transform:uppercase;"
                        "color:rgba(249,115,22,0.80);margin-bottom:6px'>" + tag + "</div>", unsafe_allow_html=True)
        with col_pct:
            st.markdown("<div style='text-align:right;font-size:0.75rem;font-weight:700;"
                        "color:rgba(249,115,22,0.80);padding-top:4px'>"
        ...
        with left_col:
            if not dietary:
                ...
                for meal, desc in dietary.items():
                    ...
                    check  = " <span style='color:#f97316'>&#10003;</span>" if done else ""
                    st.markdown(
        ...
        with right_col:
            st.markdown(
                "<div style='background:rgba(35,12,3,0.75);border:1px solid rgba(249,115,22,0.22);"
                "border-radius:12px;padding:16px 18px;margin-bottom:14px'>"
                "<div style='font-size:0.60rem;font-weight:700;letter-spacing:2px;text-transform:uppercase;"
                "color:rgba(249,115,22,0.75);margin-bottom:10px'>Today's Progress</div>"
                "<div style='display:flex;justify-content:space-between;margin-bottom:4px'>"
                "<span style='font-size:0.78rem;color:rgba(255,255,255,0.55)'>Meals completed</span>"
                "<span style='font-size:0.78rem;font-weight:700;color:#f97316'>"
            ...
            st.markdown(
                ...
                "<div style='font-size:0.60rem;font-weight:700;letter-spacing:2px;text-transform:uppercase;"
                "color:rgba(249,115,22,0.75);margin-bottom:10px'>&#128161; Nutrition Tips</div>",
                unsafe_allow_html=True)
