...
.stTabs [data-baseweb="tab"] { background:transparent!important; color:rgba(255,255,255,0.42)!important;
  border-radius:7px!important; font-size:0.78rem!important; font-weight:600!important;
...
<div style='background:linear-gradient(135deg,rgba(251,191,36,0.15),rgba(217,149,0,0.08) 40%,rgba(6,4,1,0.60));
  ...
  <div style='font-size:0.82rem;color:rgba(255,255,255,0.45)'>Track your PRs, body measurements, and progress over time</div>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    if not records:
        st.markdown("<div style='text-align:center;padding:40px'>"
                    ...
                    "<div style='font-size:1rem;color:rgba(255,255,255,0.40)'>No PRs yet. Log your first one above!</div>"
            ...
            with st.expander("🏅 " + ex_name + " — Best: " + str(best["value"]) + " " + unit, expanded=False):
                if len(ex_records) >= 2:
                    ...
                    st.markdown(
                        ...
                        + dots +
                        "<text x='0' y='77' fill='rgba(255,255,255,0.30)' font-size='7'>" + dates[0] + "</text>"
                        "<text x='200' y='77' fill='rgba(255,255,255,0.30)' font-size='7' text-anchor='end'>" + dates[-1] + "</text>"
        ...
        for ci, metric in enumerate(key_measures[:3]):
            ...
            with cols[ci % 3]:
                ...
                st.markdown(
                    ...
                    "<div style='font-size:0.60rem;color:rgba(255,255,255,0.45);margin-bottom:4px'>" + metric + "</div>"
                    "<div style='font-size:1.2rem;font-weight:700;color:#fff'>" + str(round(_vals_m[-1],1)) + " cm "
                    "<span style='font-size:0.70rem;color:" + dcol + "'>" + dsym + " " + str(round(abs(delta),1)) + "</span></div>"
                    ...
                    + dots2 +
                    "<text x='0' y='58' fill='rgba(255,255,255,0.25)' font-size='6'>" + _dates_m[0] + "</text>"
                    "<text x='120' y='58' fill='rgba(255,255,255,0.25)' font-size='6' text-anchor='end'>" + _dates_m[-1] + "</text>"
                    "</svg></div>", unsafe_allow_html=True)
    else:
        st.markdown("<div style='text-align:center;padding:40px'>"
                    "<div style='font-size:2.5rem;margin-bottom:10px'>&#128207;</div>"
                    "<div style='font-size:1rem;color:rgba(255,255,255,0.40)'>No measurements yet.</div>"
                    "</div>", unsafe_allow_html=True)
...
with tab3:
    ...
    pct_html = "".join(
        ...
        "<div style='font-size:0.60rem;color:rgba(255,255,255,0.45);margin-top:2px'>"
        + rng + " | " + zone + "</div></div>"
    ...
    st.markdown(
        ...
        " <span style='font-size:2rem;color:rgba(255,255,255,0.50)'>kg</span></div>"
        "<div style='font-size:0.75rem;color:rgba(255,255,255,0.45);margin-bottom:16px'>"
        ...
        "<div style='font-size:0.60rem;color:rgba(255,255,255,0.40);letter-spacing:1px'>EPLEY</div></div>"
        "<div style='background:rgba(255,255,255,0.06);border-radius:10px;padding:10px;text-align:center'>"
        "<div style='font-family:Bebas Neue,sans-serif;font-size:1.6rem;color:#fff'>" + str(round(brzycki,1)) + "kg</div>"
        "<div style='font-size:0.60rem;color:rgba(255,255,255,0.40);letter-spacing:1px'>BRZYCKI</div></div>"
        "<div style='background:rgba(255,255,255,0.06);border-radius:10px;padding:10px;text-align:center'>"
        "<div style='font-family:Bebas Neue,sans-serif;font-size:1.6rem;color:#fff'>" + str(round(lander,1)) + "kg</div>"
        "<div style='font-size:0.60rem;color:rgba(255,255,255,0.40);letter-spacing:1px'>LANDER</div></div>"
        "</div>"
        "<div style='margin-top:16px;padding-top:14px;border-top:1px solid rgba(255,255,255,0.08)'>"
        "<div style='font-size:0.68rem;color:rgba(255,255,255,0.40);margin-bottom:8px'>Training % targets:</div>"
...
with tab4:
    ...
    try:
        ...
        st.markdown(
            ...
            "<div style='font-size:0.68rem;color:rgba(255,255,255,0.45);letter-spacing:2px;text-transform:uppercase'>Badges Earned</div></div>"
            "<div style='text-align:right'>"
            "<div style='font-size:0.80rem;font-weight:600;color:rgba(255,255,255,0.60)'>Completion</div>"
        ...
        if next_b:
            st.markdown(
                ...
                "<div style='font-size:0.75rem;color:rgba(255,255,255,0.50);margin-top:2px'>" + next_b["desc"] + "</div>"
                "</div></div>", unsafe_allow_html=True)

        if earned:
            st.markdown("<div style='font-size:0.72rem;color:rgba(255,255,255,0.45);margin-bottom:12px'>&#127942; Earned Badges</div>", unsafe_allow_html=True)
            ...
            for bid, icon, title, desc, _ in BADGES:
                if bid not in earned_ids:
                    locked_html += (
                        ...
                        "<div style='font-size:0.72rem;font-weight:700;color:rgba(255,255,255,0.50);margin-bottom:3px'>" + title + "</div>"
                        "<div style='font-size:0.60rem;color:rgba(255,255,255,0.28);line-height:1.4'>" + desc + "</div></div>")
