...
[data-testid="stAppViewContainer"]{
  background:linear-gradient(180deg,rgba(8,5,3,0.55) 0%,rgba(8,5,3,0.45) 40%,rgba(8,5,3,0.62) 100%),
    url('https://images.unsplash.com/photo-1534438327276-14e5300c3a48?w=1800&q=80&auto=format&fit=crop')
    center center/cover no-repeat fixed!important;}
[data-testid="stAppViewContainer"]>section>div.block-container{
  max-width:1200px!important;margin:0 auto!important;padding:0 24px 80px!important;}
...
.stat-mini{background:rgba(10,6,4,0.72);border:1px solid rgba(229,9,20,0.18);
...
.stat-mini-lbl{font-size:0.58rem;font-weight:700;letter-spacing:2px;text-transform:uppercase;
  color:rgba(255,255,255,0.38);margin-top:4px;}
/* ── SECTION LABELS ── */
.sec-lbl{font-size:0.60rem;font-weight:700;letter-spacing:3px;text-transform:uppercase;
...
/* ── PANELS ── */
.g-panel{background:rgba(10,6,4,0.65);border:1px solid rgba(229,9,20,0.15);
...
.prog-lbl{font-size:0.78rem;color:rgba(255,255,255,0.55);}
...
/* ── TODAY CARD ── */
.today-card{background:rgba(229,9,20,0.08);border:1px solid rgba(229,9,20,0.25);
  border-radius:14px;padding:18px 22px;margin-bottom:10px;}
.today-date-tag{font-size:0.62rem;font-weight:700;letter-spacing:2px;text-transform:uppercase;
...
.status-chip{display:inline-block;padding:3px 12px;border-radius:20px;
  font-size:0.70rem;font-weight:700;letter-spacing:1px;text-transform:uppercase;}
...
.status-chip.pending{background:rgba(229,9,20,0.12);border:1px solid rgba(229,9,20,0.30);color:#ff6b6b;}
/* ── EXERCISE LIST ── */
...
.ex-ico{width:28px;height:28px;border-radius:6px;background:rgba(229,9,20,0.12);
  border:1px solid rgba(229,9,20,0.22);display:flex;align-items:center;
  justify-content:center;font-size:0.75rem;flex-shrink:0;}
.ex-nm{font-size:0.82rem;font-weight:600;color:#fff;flex:1;}
.ex-badges{display:flex;gap:4px;}
.ex-badge{font-size:0.60rem;font-weight:700;padding:2px 7px;border-radius:5px;}
...
.act-day{font-size:0.60rem;color:rgba(255,255,255,0.38);font-weight:600;}
/* ── CALENDAR ── */
.cal-hdr-row{display:grid;grid-template-columns:repeat(7,1fr);gap:3px;margin-bottom:4px;}
.cal-dow{font-size:0.55rem;font-weight:700;letter-spacing:1px;text-transform:uppercase;
  color:rgba(255,255,255,0.35);text-align:center;padding:4px 0;}
...
.cal-num{font-size:0.65rem;font-weight:700;color:rgba(255,255,255,0.70);}
...
/* ── PROFILE CARD ── */
.prof-card{background:rgba(10,6,4,0.70);border:1px solid rgba(229,9,20,0.18);
  border-radius:14px;padding:18px;margin-bottom:12px;}
.prof-avatar{width:44px;height:44px;border-radius:10px;background:rgba(229,9,20,0.18);
  border:1.5px solid rgba(229,9,20,0.35);display:flex;align-items:center;
...
.prof-sub{font-size:0.68rem;color:rgba(229,9,20,0.70);font-weight:600;}
.db-stat-card{background:rgba(10,6,4,0.60);border:1px solid rgba(229,9,20,0.15);
  border-radius:10px;padding:12px 14px;margin-bottom:8px;}
.db-stat-lbl{font-size:0.55rem;font-weight:700;letter-spacing:2.5px;text-transform:uppercase;
  color:rgba(255,255,255,0.35);margin-bottom:6px;}
/* ── SCHED ITEMS ── */
.sched-item{display:flex;align-items:center;gap:12px;padding:10px 14px;
  background:rgba(10,6,4,0.55);border:1px solid rgba(229,9,20,0.12);
  border-radius:10px;margin-bottom:6px;}
.sched-ico-box{width:36px;height:36px;border-radius:8px;background:rgba(229,9,20,0.12);
  border:1px solid rgba(229,9,20,0.22);display:flex;align-items:center;
  justify-content:center;font-size:1rem;flex-shrink:0;}
.sched-title{font-size:0.82rem;font-weight:600;color:#fff;}
.fit-tag{font-size:0.50rem;font-weight:700;letter-spacing:2px;text-transform:uppercase;
  color:rgba(229,9,20,0.65);margin-bottom:2px;}
.sched-sub{font-size:0.65rem;color:rgba(255,255,255,0.38);margin-top:2px;}
.sched-date{font-size:0.68rem;font-weight:700;color:rgba(229,9,20,0.70);
...
try:
    ...
    if _earned or _next:
        ...
        if _next:
            _badge_row_html += (
                "<div style='font-size:0.72rem;color:rgba(255,255,255,0.45);white-space:nowrap'>"
  ...
  <div style='display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px'>
    ...
    <div style='display:flex;gap:12px;flex-wrap:wrap;align-items:center'>
      ...
      <div style='background:rgba(229,9,20,0.12);border:1px solid rgba(229,9,20,0.28);
        border-radius:20px;padding:5px 14px;font-size:0.72rem;font-weight:700;color:#E50914'>
...
with col_left:
    ...
    if today_exercises:
        ...
        if len(today_exercises) > 8:
            ex_html += f"<div style='font-size:0.70rem;color:rgba(255,255,255,0.30);padding:6px 0 2px'>+{len(today_exercises)-8} more exercises</div>"
        ...
        st.markdown(
            ...
            "<div style='font-size:0.88rem;color:rgba(255,255,255,0.40)'>Rest Day — Recovery is progress too!</div>"
    ...
    <div class='g-panel' style='padding:16px 18px'>
      <div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:8px'>
        <span style='font-size:0.72rem;color:rgba(255,255,255,0.50)'>Plan Progress</span>
      ...
      <div style='display:flex;justify-content:space-between;margin-top:6px'>
        <span style='font-size:0.62rem;color:rgba(255,255,255,0.30)'>{done_count} done</span>
        <span style='font-size:0.62rem;color:rgba(255,255,255,0.30)'>{plan_duration - done_count} remaining</span>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        f"<div style='font-size:0.80rem;color:rgba(255,255,255,0.65);line-height:1.65;"
        f"font-style:italic;margin-bottom:6px'>&ldquo;{_q}&rdquo;</div>"
        f"<div style='font-size:0.62rem;font-weight:700;color:rgba(229,9,20,0.65);"
...
with col_mid:
    ...
    <div class='g-panel' style='padding:16px 18px'>
      <div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:10px'>
        <div style='font-size:0.78rem;color:rgba(255,255,255,0.55)'>{week_msg}</div>
        <div style='font-family:Bebas Neue,sans-serif;font-size:1.8rem;color:#E50914;line-height:1'>
          {week_done}<span style='font-size:0.9rem;color:rgba(255,255,255,0.30)'>/7</span></div>
      </div>
    ...
    st.markdown(
        ...
        f"<div style='font-size:0.52rem;color:rgba(255,255,255,0.40);letter-spacing:1.5px;text-transform:uppercase'>Done</div></div>"
        ...
        f"<div style='font-size:0.52rem;color:rgba(255,255,255,0.40);letter-spacing:1.5px;text-transform:uppercase'>Skipped</div></div>"
        ...
        f"<div style='font-size:0.52rem;color:rgba(255,255,255,0.40);letter-spacing:1.5px;text-transform:uppercase'>Left</div></div>"
        f"</div>",
        unsafe_allow_html=True
    )
    ...
    if _wlog and len(_wlog) >= 2:
        ...
        st.markdown(f"""
<div style='background:rgba(10,6,4,0.55);border:1px solid rgba(229,9,20,0.18);border-radius:12px;padding:12px 14px;margin-top:8px'>
  <div style='display:flex;justify-content:space-between;margin-bottom:8px'>
    <span style='font-size:0.60rem;color:rgba(255,255,255,0.38);letter-spacing:2px;text-transform:uppercase'>Weight History</span>
    <span style='font-size:0.78rem;font-weight:700;color:{_dcol}'>{_dsym} {abs(_delta):.1f} kg</span>
  </div>
  <svg viewBox="0 0 200 100" style="width:100%">
    ...
    {_dots}
    <text x="0" y="95" fill="rgba(255,255,255,0.30)" font-size="7">{_dates[0]}</text>
    <text x="180" y="95" fill="rgba(255,255,255,0.30)" font-size="7" text-anchor="end">{_dates[-1]}</text>
  </svg>
</div>""", unsafe_allow_html=True)
    ...
    if missed:
        ...
        for cd in missed[:3]:
            ...
            with m1:
                st.markdown(
                    f"<div style='font-size:0.78rem;color:rgba(255,255,255,0.60);padding:7px 0;"
                    f"display:flex;align-items:center;gap:8px'>"
                    f"<span style='font-size:0.62rem;color:rgba(239,68,68,0.70);font-weight:700'>{_days_ago}d ago</span>"
...
with col_right:
    ...
    st.markdown(
        ...
        f"<span style='font-size:0.80rem;font-weight:700;color:rgba(255,255,255,0.65)'>{month_label}</span>"
        f"<span style='font-size:0.60rem;color:rgba(255,255,255,0.25);letter-spacing:1px'>"
...
with row3_left:
    ...
    if upcoming:
            ...
            for _col_widget, cd in zip([up_c1, up_c2], pair):
                with _col_widget:
                    ...
                    st.markdown(
                        ...
                        f"<div>"
                        f"<div style='font-size:0.48rem;font-weight:700;letter-spacing:2px;text-transform:uppercase;"
                        f"color:{_accent};margin-bottom:2px'>{_mg2.split()[0].upper()[:8] if not _rest else 'REST'}</div>"
                        f"<div style='font-size:0.82rem;font-weight:700;color:#fff;line-height:1.2'>{_title_txt[:20]}</div>"
                        f"</div></div>"
                        f"<div style='text-align:right'>"
                        f"<div style='font-size:0.70rem;font-weight:700;{_date_style}'>{_dl}</div>"
                        f"<div style='font-size:0.58rem;color:rgba(255,255,255,0.28);margin-top:2px'>Day {cd['day_idx']+1}</div>"
                        f"</div></div>"
                        f"<div style='font-size:0.62rem;color:rgba(255,255,255,0.40)'>{_sub_txt}</div>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
    else:
        st.markdown(
            ...
            "<div style='font-size:2rem;margin-bottom:8px'>🎉</div>"
            "<div style='font-size:0.90rem;font-weight:700;color:#4ade80'>All workouts complete!</div>"
            "<div style='font-size:0.72rem;color:rgba(255,255,255,0.35);margin-top:4px'>You crushed this plan!</div>"
            "</div>",
            unsafe_allow_html=True
        )
        ...
        for _col_w, (_ico, _lbl, _desc, _path, _key) in zip(_nc, _ni_row):
            with _col_w:
                st.markdown(
                    ...
                    f"<div style='font-size:0.72rem;font-weight:700;color:#fff;letter-spacing:0.3px'>{_lbl}</div>"
                    f"<div style='font-size:0.55rem;color:rgba(255,255,255,0.30);margin-top:3px;line-height:1.3'>{_desc}</div>"
                    f"</div>",
                    unsafe_allow_html=True
                )
